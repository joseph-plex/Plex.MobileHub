...
namespace Plex.PMH.Repositories
{
    public class Connections
    {
        ...
        //    var clients = Access.Clients.GetAll();
        //    var index = clients.FindIndex((p) => p.ClientId == Data.ClientId);

        //    //Ensure data is all valid
        //    if(index == -1) throw new InvalidClientException();
        //    if (clients[index].ClientKey != Data.Key) throw new Exception("Invalid Client Key");
        ...
        //{
        //    if (!Access.Clients.GetAll().Exists((p) => p.ClientId == Data.ClientId && p.ClientKey == Data.Key))
